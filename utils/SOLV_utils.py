...
from externals.SOLV.datasets import ytvis19
from datasets import ssv2_SOLV
...
def get_dataloaders(args):
    ...
    if args.dataset == "ytvis19":
        ...
        train_dataset = ssv2_SOLV.SSV2(args, "train", sa_labels)
        val_dataset = ssv2_SOLV.SSV2(args, "val", sa_labels)
...
def get_ari(prediction_masks, gt_masks, bg_class):
    ...
    prediction_masks = (
        torch.flatten(prediction_masks, start_dim=1, end_dim=-1)
        .cpu()
        .numpy()
        .astype(int)
    )
    gt_masks = (
        torch.flatten(gt_masks, start_dim=1, end_dim=-1).cpu().numpy().astype(int)
    )

    assert (
        prediction_masks.shape == gt_masks.shape
    ), f"prediction_masks.shape: {prediction_masks.shape} gt_masks.shape: {gt_masks.shape}"
...
class Evaluator:
    ...
    def calculate_miou(self, pred, gt):
        ...
        assert (
            len(pred_classes) <= len(gt_classes) + 1
        ), f"pred_classes: {pred_classes}, gt_classes: {gt_classes}"
    ...
    def update(self, prediction_masks, gt_masks):
        ...
        assert (
            gt_masks.shape == prediction_masks.shape
        ), f"gt_masks.shape: {gt_masks.shape}, prediction_masks.shape: {prediction_masks.shape}"
